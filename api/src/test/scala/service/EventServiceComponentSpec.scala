...
class EventServiceComponentSpec extends AsyncFlatSpec
  with AsyncIOSpec
  with Matchers
  with EventServiceComponent
  with MockEventRepositoryComponent {
  override val eventService: EventService = new EventService {}

  behavior of "An EventService"

  it should "return an empty list when name is not found" in {
    eventService.findEvents("Albert Einstein", null).asserting { events =>
      events shouldBe empty
    }
  }

  it should "return all events for a given name" in {
